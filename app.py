...
from utils import (analyze_drawing, generate_pdf, extract_title_block,
                   analyze_gdt, analyze_design_concerns, analyze_material,
                   analyze_manufacturing, detect_dimensions)
...
LIBRARY_FILE      = "drawing_library.json"
LIBRARY_DIR       = "drawing_library"
MAX_CHATS         = 20
MAX_FILE_SIZE_MB  = 10
...
def save_rate_limits(d):
    with open(RATE_LIMIT_FILE, "w") as f: json.dump(d, f)
...
def check_rate_limit(ip):
    lim = load_rate_limits(); now = time.time()
    if ip not in lim: lim[ip] = {"count": 0, "window_start": now}
    e = lim[ip]
    if now - e["window_start"] > 3600: e["count"] = 0; e["window_start"] = now
    if e["count"] >= MAX_REQUESTS_PER_IP: save_rate_limits(lim); return False, 0
    return True, MAX_REQUESTS_PER_IP - e["count"]

def increment_rate_limit(ip):
    lim = load_rate_limits(); now = time.time()
    if ip not in lim: lim[ip] = {"count": 0, "window_start": now}
    lim[ip]["count"] += 1; save_rate_limits(lim)
...
def save_library(lib):
    with open(LIBRARY_FILE, "w") as f: json.dump(lib, f)
...
def add_to_library(uploaded_file, tags="", notes=""):
    ...
    with open(dest, "wb") as f: f.write(uploaded_file.read())
    uploaded_file.seek(0)
    lib[uid] = {
        "name":    name,
        "uid":     uid,
...
def fmt(text):
    ...
    for line in lines:
        ...
        if s.startswith("### ") or s.startswith("## "):
            if in_list: html += f'</{in_list}>'; in_list = False
            h = re.sub(r'^#+\s*','',s); h = re.sub(r'\*\*(.*?)\*\*',r'<strong>\1</strong>',h)
            html += f'<p style="margin:10px 0 4px;font-weight:600;color:#fff;font-size:14px;">{h}</p>'
        elif re.match(r'^\d+\.',s):
            if in_list!="ol":
                if in_list: html+=f'</{in_list}>'
                html+='<ol style="margin:4px 0 4px 20px;padding:0;color:#fff;">'; in_list="ol"
            item=re.sub(r'^\d+\.\s*','',s); item=re.sub(r'\*\*(.*?)\*\*',r'<strong>\1</strong>',item)
            ...
            if in_list!="ul":
                if in_list: html+=f'</{in_list}>'
                html+='<ul style="margin:4px 0 4px 20px;padding:0;color:#fff;">'; in_list="ul"
            item=re.sub(r'^[-•○◦]\s*','',s); item=re.sub(r'\*\*(.*?)\*\*',r'<strong>\1</strong>',item)
            html+=f'<li style="margin-bottom:4px;line-height:1.7;font-size:14px;">{item}</li>'
        else:
            if in_list: html+=f'</{in_list}>'; in_list=False
...
def render_dim_table(json_str):
    """Render dimension detection JSON as an HTML table."""
    try:
        ...
        if not dims: return fmt(json_str)
        rows = ""
        ...
        sum_row = f'<div style="padding:8px 12px;font-size:11px;color:rgba(255,255,255,0.35);border-top:1px solid rgba(255,255,255,0.06);">{summary}</div>' if summary else ""
...
def render_title_block(raw):
    ...
    for line in raw.strip().split("\n"):
        if ":" in line:
            parts = line.split(":", 1)
            k = parts[0].strip(); v = parts[1].strip() if len(parts)>1 else "—"
            if v and v.lower() != "not specified":
                rows += f'<tr><td style="padding:7px 14px;color:rgba(255,255,255,0.45);font-size:12px;font-family:JetBrains Mono,monospace;white-space:nowrap;border-bottom:1px solid rgba(255,255,255,0.04);">{k}</td><td style="padding:7px 14px;color:#fff;font-size:13px;border-bottom:1px solid rgba(255,255,255,0.04);">{v}</td></tr>'
    return f'''<div style="background:rgba(249,115,22,0.05);border:1px solid rgba(249,115,22,0.18);border-radius:10px;overflow:hidden;">
        <div style="padding:8px 14px;font-size:10px;font-family:JetBrains Mono,monospace;color:#f97316;letter-spacing:2px;text-transform:uppercase;border-bottom:1px solid rgba(249,115,22,0.12);">🏷️ TITLE BLOCK</div>
...
# PAGE CONFIG
# ══════════════════════════════════════════
st.set_page_config(page_title="DrawingAI", page_icon="⚙️", layout="wide", initial_sidebar_state="expanded")
...
[data-testid="stAppViewContainer"]::before {
    ...
    background: radial-gradient(ellipse 50% 40% at 20% 20%,rgba(249,115,22,0.04) 0%,transparent 70%),
                radial-gradient(ellipse 40% 30% at 80% 80%,rgba(249,115,22,0.03) 0%,transparent 70%);
    pointer-events:none; z-index:0;
}
.block-container { max-width:880px !important; margin:0 auto !important; padding:0 24px 180px !important; }
.stDeployButton,#MainMenu,footer { display:none !important; }

/* SIDEBAR TOGGLE */
[data-testid="stHeader"] { background:transparent !important; height:0 !important; overflow:visible !important; position:relative !important; z-index:9999 !important; }
[data-testid="collapsedControl"] { position:fixed !important; top:16px !important; left:16px !important; z-index:9999 !important; display:flex !important; visibility:visible !important; opacity:1 !important; background:rgba(249,115,22,0.15) !important; border:1px solid rgba(249,115,22,0.4) !important; border-radius:8px !important; padding:7px 9px !important; cursor:pointer !important; }
[data-testid="collapsedControl"]:hover { background:rgba(249,115,22,0.28) !important; }
[data-testid="collapsedControl"] svg { color:#f97316 !important; fill:#f97316 !important; width:18px !important; height:18px !important; }

/* SIDEBAR */
[data-testid="stSidebar"] { background:#0f0f0f !important; border-right:1px solid rgba(255,255,255,0.05) !important; }
[data-testid="stSidebar"] > div:first-child { padding:24px 16px !important; }
.sb-logo { font-family:'JetBrains Mono',monospace; font-size:14px; font-weight:500; color:#f97316; margin-bottom:2px; }
.sb-sub  { font-size:10px; color:rgba(255,255,255,0.2); margin-bottom:16px; font-family:'JetBrains Mono',monospace; }
.sb-label{ font-family:'JetBrains Mono',monospace; font-size:9px; letter-spacing:2px; text-transform:uppercase; color:rgba(255,255,255,0.18); margin-bottom:8px; margin-top:14px; }
...
[data-testid="stSidebar"] .stButton > button { background:rgba(255,255,255,0.03) !important; border:1px solid rgba(255,255,255,0.06) !important; color:rgba(255,255,255,0.6) !important; border-radius:8px !important; font-size:12px !important; padding:9px 12px !important; width:100% !important; text-align:left !important; margin-bottom:4px !important; transition:all 0.15s !important; }
[data-testid="stSidebar"] .stButton > button:hover { background:rgba(249,115,22,0.08) !important; border-color:rgba(249,115,22,0.2) !important; color:#f97316 !important; }

/* TOP NAV */
.top-nav { display:flex; align-items:center; justify-content:space-between; padding:16px 0 12px 48px; border-bottom:1px solid rgba(255,255,255,0.05); margin-bottom:12px; position:sticky; top:0; background:rgba(13,13,13,0.96); backdrop-filter:blur(10px); z-index:100; }
.nav-title { font-size:20px; font-weight:700; color:#fff; display:flex; align-items:center; gap:8px; }
...
/* FILE UPLOADER */
[data-testid="stFileUploader"] > div { border:1.5px dashed rgba(255,255,255,0.08) !important; background:rgba(255,255,255,0.02) !important; border-radius:8px !important; padding:8px 14px !important; }
...
.section-label { font-family:'JetBrains Mono',monospace; font-size:9px; letter-spacing:2px; text-transform:uppercase; color:rgba(255,255,255,0.2); margin-bottom:7px; }

/* BUTTONS */
.stButton > button { background:rgba(255,255,255,0.03) !important; border:1px solid rgba(255,255,255,0.07) !important; color:#fff !important; border-radius:7px !important; font-family:'Inter',sans-serif !important; font-size:11px !important; padding:7px 10px !important; width:100% !important; transition:all 0.15s !important; text-align:left !important; }
.stButton > button:hover { background:rgba(249,115,22,0.08) !important; border-color:rgba(249,115,22,0.25) !important; color:#f97316 !important; }
.stButton > button[kind="primary"] { background:#f97316 !important; border:none !important; color:#fff !important; font-weight:600 !important; font-size:14px !important; border-radius:10px !important; text-align:center !important; padding:13px !important; }
...
.bubble-user { background:rgba(249,115,22,0.11); border:1px solid rgba(249,115,22,0.2); border-radius:18px 18px 4px 18px; padding:11px 15px; max-width:72%; font-size:14px; color:#fff; line-height:1.65; }
.ai-avatar { width:28px; height:28px; flex-shrink:0; background:rgba(249,115,22,0.12); border:1px solid rgba(249,115,22,0.2); border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:13px; margin-top:2px; }
.bubble-ai { max-width:88%; font-size:14px; color:#fff; line-height:1.7; }
.chat-empty { text-align:center; padding:60px 0 40px; color:rgba(255,255,255,0.12); font-family:'JetBrains Mono',monospace; font-size:11px; letter-spacing:1.5px; text-transform:uppercase; }

/* STICKY INPUT */
.sticky-wrap { position:fixed; bottom:0; left:0; right:0; z-index:200; background:rgba(13,13,13,0.97); border-top:1px solid rgba(255,255,255,0.07); backdrop-filter:blur(12px); padding:12px 0 16px; }
.sticky-inner { max-width:880px; margin:0 auto; padding:0 24px; }
.stTextArea textarea { background:rgba(255,255,255,0.05) !important; border:1px solid rgba(255,255,255,0.1) !important; border-radius:12px !important; color:#fff !important; font-family:'Inter',sans-serif !important; font-size:14px !important; padding:12px 16px !important; resize:none !important; line-height:1.6 !important; }
...
/* DOWNLOAD BUTTON */
.stDownloadButton button { background:rgba(255,255,255,0.05) !important; border:1px solid rgba(255,255,255,0.1) !important; color:#fff !important; border-radius:10px !important; font-size:12px !important; padding:13px 8px !important; width:100% !important; font-weight:500 !important; }
.stDownloadButton button:hover { background:rgba(249,115,22,0.1) !important; border-color:rgba(249,115,22,0.3) !important; color:#f97316 !important; }

/* TEXT INPUT */
.stTextInput input { background:rgba(255,255,255,0.05) !important; border:1px solid rgba(255,255,255,0.08) !important; border-radius:8px !important; color:#fff !important; font-size:13px !important; padding:8px 12px !important; }
...
/* LIBRARY CARD */
.lib-card { background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.07); border-radius:10px; padding:12px 14px; margin-bottom:10px; transition:border-color 0.15s; }
...
# SESSION STATE
# ══════════════════════════════════════════
for k, v in [("chat_history",[]),("messages_display",[]),("current_drawing_name",None),
             ("title_block_data",None),("active_tab","analyze")]:
...
# ══════════════════════════════════════════
with st.sidebar:
    st.markdown('<div class="sb-logo">⚙️ DrawingAI</div>', unsafe_allow_html=True)
    ...
    if st.button("📚  Drawing Library", use_container_width=True):
        st.session_state.active_tab = "library"; st.rerun()
    ...
    if not st.session_state.saved_chats:
        st.markdown('<div style="font-size:11px;color:rgba(255,255,255,0.12);font-family:\'JetBrains Mono\',monospace;padding:4px 0;">No chats yet.</div>', unsafe_allow_html=True)
    else:
        for name in reversed(list(st.session_state.saved_chats.keys())):
            ...
            with cd:
                if st.button("✕", key=f"del_{name}"):
                    ...
                    save_chats(st.session_state.saved_chats); st.rerun()
...
st.markdown(f"""
<div class="top-nav">
    <div class="nav-title"><span class="nav-icon">⚙️</span>&nbsp;Drawing<span>AI</span></div>
    <div class="nav-right">
        <div class="nav-badge">{tab_label}</div>
        <div class="nav-badge" style="color:rgba(255,255,255,0.3);border-color:rgba(255,255,255,0.1);">Beta</div>
    </div>
...
if st.session_state.active_tab == "library":
    ...
    add_file = st.file_uploader("Add drawing", type=["png","jpg","jpeg","webp"], label_visibility="collapsed", key="lib_upload")
    ...
    search = st.text_input("Search", placeholder="Search by name or tag...", label_visibility="collapsed", key="lib_search")

    lib = load_library()  # reload after possible add
    if not lib:
        st.markdown('<div class="chat-empty"><div style="font-size:30px;opacity:0.15;margin-bottom:8px;">📚</div><div>No drawings saved yet</div></div>', unsafe_allow_html=True)
    else:
        # Filter by search
        filtered = {k: v for k, v in lib.items()
                    if not search or search.lower() in v["name"].lower()
                    or any(search.lower() in t.lower() for t in v.get("tags",[]))}

        st.markdown(f'<div style="font-size:11px;color:rgba(255,255,255,0.25);font-family:JetBrains Mono,monospace;margin-bottom:10px;">{len(filtered)} drawing{"s" if len(filtered)!=1 else ""} found</div>', unsafe_allow_html=True)

        for uid, meta in reversed(list(filtered.items())):
            with st.container():
                tags_html = "".join(f'<span class="lib-tag">{t}</span>' for t in meta.get("tags",[]))
                notes_txt = f'<div style="font-size:11px;color:rgba(255,255,255,0.25);margin-top:4px;">{meta["notes"]}</div>' if meta.get("notes") else ""
                ...
                with c2:
                    ...
                    try:
                        with open(meta["path"], "rb") as f:
                            file_bytes = f.read()
                        st.download_button("⬇️ Download", data=file_bytes,
                                           file_name=meta["name"], use_container_width=True, key=f"dl_{uid}")
                    except:
                ...
                with c3:
                    if st.button("🗑️", key=f"libdel_{uid}", use_container_width=True, help="Remove from library"):
                        delete_from_library(uid); st.rerun()
...
# ══════════════════════════════════════════════════════
    ...
    if not st.session_state.messages_display:
        st.markdown("""<div class="chat-empty">
            ...
            <div>Upload a drawing and start asking</div>
        </div>""", unsafe_allow_html=True)
    else:
        for msg in st.session_state.messages_display:
            if msg["role"] == "user":
                ...
                if content.startswith("__TB__"):
                    ...
                    st.markdown(f'<div class="msg-row ai"><div class="ai-avatar">⚙️</div><div class="bubble-ai" style="max-width:90%;">{bubble}</div></div>', unsafe_allow_html=True)
                elif content.startswith("__DIM__"):
                    bubble = render_dim_table(content[7:])
                    st.markdown(f'<div class="msg-row ai"><div class="ai-avatar">⚙️</div><div class="bubble-ai" style="max-width:95%;">{bubble}</div></div>', unsafe_allow_html=True)
                else:
                    st.markdown(f'<div class="msg-row ai"><div class="ai-avatar">⚙️</div><div class="bubble-ai">{fmt(content)}</div></div>', unsafe_allow_html=True)

    st.markdown('<div class="footer-txt" style="margin-bottom:8px;">Made with <span>♥️</span> by Rishi</div>', unsafe_allow_html=True)

    # ── STICKY BOTTOM INPUT ──
    st.markdown('<div class="sticky-wrap"><div class="sticky-inner">', unsafe_allow_html=True)
    custom_q = st.text_area("msg", placeholder="Ask anything about the drawing...", label_visibility="collapsed", height=52)
    col_ask, col_clear, col_pdf = st.columns([5, 1, 1])
    with col_ask:
    ...
    with col_pdf:
        if st.session_state.messages_display:
            pdf_buf = generate_pdf(st.session_state.messages_display,
                                   drawing_name=st.session_state.current_drawing_name or "drawing",
                                   title_block_data=st.session_state.title_block_data)
            st.download_button("📄 PDF", data=pdf_buf, file_name="drawing_analysis.pdf",
